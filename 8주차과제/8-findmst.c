...
void set_init(int n)
{
    for (int i = 0; i < n; i++) // n까지 초기화하도록 수정
        parent[i] = -1;
}
...
void QuickKruskal(GraphType *g)
{
    ...
    set_init(g->vertex_count + 1); // 정점의 개수만큼 초기화
    qsort(g->edges, g->n, sizeof(struct Edge), compare);

    printf("Quick Based Kruskal \n");
    int i = 0;

    while (edge_accepted < (g->vertex_count - 1))
    { // 정점 개수 기준으로 변경
        e = g->edges[i];
        ...
        if (uset != vset)
        { // 서로 속한 집합이 다르면
            printf("Edge (%d,%d) selected with weight %d \n", e.start, e.end, e.weight);
            edge_accepted++;
...
void MinHeapKruskal(GraphType *g)
{
    ...
    set_init(g->vertex_count + 1);
    init_min_heap(&min_heap);

    for (int i = 0; i < g->n; i++)
        insert_min_heap(&min_heap, g->edges[i]);

    printf("Min-Heap Based Kruskal\n");

    while (edge_accepted < (g->vertex_count - 1))
    {
        e = delete_min_heap(&min_heap);
...
int main(void)
{
    ...
    GenerateGraph(g);
    QuickKruskal(g);   // quick 기반 Kruskal 실행
